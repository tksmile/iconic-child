...
/* 独自追加-- class AbplusSpecを利用するため */
require_once( STYLESHEETPATH. '/customize/abplus_spec.php' );
...
function usces_pdf_out( $pdf, $data ) {
	...
	// Template file 独自変更-- テンプレート使ってないので削除
	...
	for( $index = 0; $index < $cart_count; $index++ ) {
		...
		$pdf->MultiCell( 4, $lineheight, '*', 0, 'C' ); //商品名先頭の*印
		$pdf->SetXY( $x + 3.0, $line_y );
		$pdf->MultiCell( 84.6, $lineheight, usces_conv_euc( apply_filters( 'usces_filter_cart_item_name_nl', $cartItemName, $args ) ), 0, 'L' ); //商品名
		...
		/* 独自変更 */
		//商品仕様の値を配列で取得
		$spec_values = $Spec->get_values( $cart_row['advance'][0] );

		//数量は位置変更
		//$pdf->MultiCell( 11.5, $lineheight, usces_conv_euc( $cart_row['quantity'] ), 0, 'R' );

		//数量に変えて通常単価を
		$pdf->MultiCell( 15.3, $lineheight, usces_conv_euc( $spec_values['通常単価'] ), 0, 'R' );
		/* ↑独自変更 */

		list( $fontsize, $lineheight, $linetop ) = usces_set_font_size( $fontsizes['unit'] );
		$pdf->SetFont( $font, '', $fontsize );
		$pdf->SetXY( $x + 103.3, $line_y ); //org:$x + 99.6
		...
		//単位に変えて商品入数を
		$pdf->MultiCell( 15.2, $lineheight, usces_conv_euc( $spec_values['商品入数'] ), 0, 'R' );
		/* ↑独自変更 */

		$pdf->SetXY( $x + 118.6, $line_y );
		...
		//価格に変えて(位置変更の)数量を
		$pdf->MultiCell( 11.3, $lineheight, usces_conv_euc( $cart_row['quantity'] ), 0, 'R' );
		/* ↑独自変更 */

		$pdf->SetXY( $x + 130.1, $line_y );
		list( $fontsize, $lineheight, $linetop ) = usces_set_font_size( $fontsizes['row_price'] );
		$pdf->SetFont( $font, '', $fontsize );
		$pdf->MultiCell( 17.7, $lineheight, apply_filters( 'usces_filter_cart_row_price_pdf', usces_conv_euc( $usces->get_currency( $cart_row['price'] * $cart_row['quantity'] ) ), $cart_row ), 0, 'R' ); //ord-width:22.8
	...
	} //end for( $index = 0; $index < $cart_count; $index++ )
...
function usces_pdfSetHeader( $pdf, $data, $page, $font ) {
	...
	if( 'receipt' == $_REQUEST['type'] ) { //入金済み？
	...
	} elseif( 'nohin' == $_REQUEST['type'] ) { //納品書
		//「配送先を宛名とする」
		...
		list( $fontsize, $lineheight, $linetop ) = usces_set_font_size( $fontsizes['order_date'] );
		$pdf->SetFont( $font, '', $fontsize );
		$y = 89; //1行前のorder dateは84
		$pdf->SetXY( $leftside, $y );
		$pdf->MultiCell( 75, $lineheight, usces_conv_euc( '配送希望日 : '. $data->order['delivery_date'] ), 0, 'L' );

		//delivery_time
		$pdf->SetXY( $leftside + 68, $y );
		$pdf->Cell( 75, $lineheight, usces_conv_euc( '配送希望時間 : '. $data->order['delivery_time'] ), 0, 1, 'L' );
		/* ↑独自追加 */
...
function usces_pdfSetFooter( $pdf, $data, $font, $endflag ) {
	...
	$pdf->SetLineWidth( 0.5 );
	//$pdf->Line( $line_left, $line_top, $line_left, $line_bottom ); //左端線消す
	$pdf->SetLineWidth( 0.04 );
	$pdf->Line( 103.5, $line_top, 103.5, $line_footertop ); //商品名と通常単価の区切線
	$pdf->SetLineWidth( 0.5 );
	$pdf->Line( 103.5, $line_footertop, 103.5, $line_bottom );
	$pdf->SetLineWidth( 0.04 );
	$pdf->Line( 119.7, $line_top, 119.7, $line_footertop ); //org-x-top:115.5, org-x-bottom:115.5, 通常単価と商品入数の区切線
	$pdf->Line( 134.7, $line_top, 134.7, $line_footertop ); //org-x-top:127, org-x-bottom:127, 商品入数と数量の区切線
	$pdf->Line( 145.8, $line_top, 145.8, $line_bottom ); //org-x-top:142.5, org-x-bottom:142.5, 数量と金額の区切線
	$pdf->SetLineWidth( 0.5 );
	//$pdf->Line( $line_right, $line_top, $line_right, $line_bottom ); //右端線消す
	...
	/* 独自変更-- 受注商品リストの項目名を変更 */
	$pdf->MultiCell( 15.3, $lineheight, usces_conv_euc( '通常単価' ), 0, 'C' ); //ort-width:11.4, org-text:__( 'Quant', 'usces' )
	$pdf->SetXY( 119.7, 94.9 ); //ort-x:115.8
	$pdf->MultiCell( 15.3, $lineheight, usces_conv_euc( '商品入数' ), 0, 'C' ); //ort-width:11.0, org-text:__( 'Unit', 'usces' )
	$pdf->SetXY( 134.4, 94.9 ); //ort-x:127.2
	$pdf->MultiCell( 11.5, $lineheight, usces_conv_euc( __( 'Quant', 'usces' ) ), 0, 'C' ); //ort-width:15.0, org-text:__( 'Price', 'usces' )
	$pdf->SetXY( 145.3, 94.9 ); //ort-x:142.9
	$pdf->MultiCell( 17.6, $lineheight, usces_conv_euc( __( 'Amount', 'usces' ) . '(' . __( usces_crcode( 'return' ), 'usces' ) . ')' ), 0, 'C' ); //ort-width:22.4
